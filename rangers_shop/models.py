...
class Product(db.Model):
    ...
    def set_image(self, image, name):
        if not image: 
            image = get_image(name) #adding get_image function which makes an external API call
...
class Customer(db.Model):
    ...
    prod_order = db.relationship('ProdOrder', backref = 'customer', lazy = True) #backref is how are these related, lazy means a customer can exist without the ProdOrder table
...
class Order(db.Model):
    ...
    date_created = db.Column(db.DateTime, default = datetime.utcnow())
    preorder = db.relationship('ProdOrder', backref = 'order', lazy = True)
...
products_schema = ProductSchema(many = True) # this is for passing mulitple products, list of dictionaries