...
def get_order(cust_id):
    ...
    for order in prodorder:
        ...
        prod_data['id'] = order.prod_order_id #need to make products unique even if they are the same product
...
def create_order(cust_id):
    ...
    #looping through the cutomser order list of dictionaries for each product
...
#create our DELETE route for our order, associated 
...
def delete_item_order(order_id):
    ...
    order.decrement_order_total(prodorder.price) #order total is going to be less expensive
